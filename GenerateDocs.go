...
import (
	...
	beeLogger "github.com/sage-z/beego2ts/logger"
)
		...
		filepath.Walk(dirpath, func(fpath string, fileInfo os.FileInfo, err error) error {
			...
			if !(d == "vendor" || strings.HasPrefix(d, "vendor"+string(os.PathSeparator))) &&
				...
				if err != nil {
					...
					c <- fmt.Errorf("解析目录时出错: %s", err.Error())
					return nil
				}
			}
			return nil
		...
		for _, c := range f.Comments {
			for _, s := range strings.Split(c.Text(), "\n") {
				if strings.HasPrefix(s, "@APIVersion") {
					...
					if len(p) < 2 {
						beeLogger.Log.Fatalf("没有足够的参数来保证安全: %d\n", len(p))
					}
					out.Type = p[1]
					switch out.Type {
					case "oauth2":
					...
					case "apiKey":
						if len(p) < 4 {
							beeLogger.Log.Fatalf("没有足够的参数给apiKey: %d\n", len(p))
						}
						if !(p[3] == "header" || p[3] == "query") {
							beeLogger.Log.Fatalf("类型未知:%s。可能的值是`query`或`header`.\n", p[4])
						}
						out.Name = p[2]
						out.In = p[3]
						if len(p) > 4 {
					...
					default:
						beeLogger.Log.Fatalf("未知的安全类型:%s。可能的值是`oauth2`、`apiKey`或`basic`.\n", p[1])
					}
...
func analyseControllerPkg(vendorPath, localName, pkgpath string) {
	...
	gopaths := GetGOPATHs()
	if len(gopaths) == 0 {
		beeLogger.Log.Fatal("GOPATH环境变量未设置或为空")
	}
	pkgRealpath := ""
